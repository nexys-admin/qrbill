...
test("validateBoolean address", () => {
  expect(
    Validate.validateBoolean(sampleAddressJson, Validate.sAddress)
  ).toEqual(true);
});
...
test("validateBoolean cdtr", () => {
  expect(
    Validate.validateBoolean(
      { IBAN: "CH4431999123000889012", Cdtr: sampleAddressJson },
      Validate.vCdtrInf
    )
  ).toEqual(true);
});
...
test("validateBoolean ccyamt", () => {
  expect(
    Validate.validateBoolean({ Amt: "23.02", Ccy: "CHF" }, Validate.vCcyAmt)
  ).toEqual(true);
});
...
test("vAltPmtInf", () => {
  ...
  expect(Validate.validateBoolean(AltPmtInf, Validate.vAltPmtInf)).toEqual(
    true
  );
});
